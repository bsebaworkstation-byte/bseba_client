...
const AddDamage = () => {
  ...
  const handleSubmit = async () => {
    ...
    const invalidQty = selectedProducts.find((p) => !p.qtySold || p.qtySold <= 0);
    if (invalidQty) {
    ...
    const payload = {
      ...
      DamageProduct: selectedProducts.map((p) => ({
        productID: p._id,
  ...
  return (
    <div className="global_container ">
      <h4 className="global_heading mb-4">{heading("addDamage")}</h4>
      {/* Product Selection */}
            ...
            <tbody className="global_tbody">
              {selectedProducts.map((p, idx) => (
                <tr key={p._id + "-" + idx} className="global_tr">
                  ...
                  {/* Sale Price - read only for damage */}
                  <td className="global_td w-24">
                    {(p.price || 0).toFixed(2)}
                  </td>
                  {/* DP*/}
