...
const EditSale = () => {
  ...
  const [refreshingStockIdx, setRefreshingStockIdx] = useState(-1);
  // language translator
  ...
  const getStockProductId = (rowOrId) => {
    if (!rowOrId) return "";
    if (typeof rowOrId === "string") return rowOrId.trim();
    return (rowOrId.productID || rowOrId._id || rowOrId.value || "").trim();
  };

  const formatStockOptionLabel = (stockQty, createdDate) =>
    `${stockQty ?? 0} (${formatDate(createdDate)})`;

  const formatStockOptions = (data) =>
    (Array.isArray(data) ? data : []).map((s) => ({
      label: formatStockOptionLabel(s.stock, s.CreatedDate),
      value: s._id,
      productID: s.productID,
      ...s,
    }));

  const fetchProductStocksByProductID = async (
    productId,
    { silent = false } = {},
  ) => {
    const id = getStockProductId(productId);
    if (!id) return [];
    ...
    try {
      ...
      if (res.data.status === "Success") {
        return formatStockOptions(res.data.data);
      }
      return [];
    } catch (error) {
      console.error(error);
      if (!silent) ErrorToast("Failed to load stock");
      return [];
    } finally {
  ...
  const getAvailableStockList = (allStocks, productId, rowIndex, rows) => {
    ...
    return allStocks.filter(
      (s) =>
        !rows.some(
          (p, i) =>
            i !== rowIndex &&
            getStockProductId(p) === productId &&
            p.productLineID === s._id,
        ),
    );
  };

  const buildStockOptionFromSaleLine = (row) => ({
    label: formatStockOptionLabel(
      row.stock ?? row.qtySold ?? 0,
      row.CreatedDate || purchaseDate,
    ),
    value: row.productLineID,
    _id: row.productLineID,
    ...
    productCode: row.productCode || "0",
  });

  const ensureSelectedInStockOptions = (available, selectedStock) => {
    if (!selectedStock?.[0]) return available;
    const selectedId = selectedStock[0]._id || selectedStock[0].value;
    if (available.some((s) => s._id === selectedId || s.value === selectedId)) {
      return available;
    }
    return [selectedStock[0], ...available];
  };
  ...
  const applyStockToRow = (row, allStocks, rowIndex, rows) => {
    ...
    let selected =
      allStocks.find((s) => s._id === row.productLineID) ||
      availableStockList[0] ||
      null;

    if (!selected && row.productLineID) {
      selected = buildStockOptionFromSaleLine(row);
    }

    if (!selected) {
      return {
        ...row,
        manageStock: row.manageStock ?? 0,
        stocks: [],
        selectedStock: [],
      };
    }

    const stockOptions = ensureSelectedInStockOptions(availableStockList, [
      selected,
    ]);
    ...
    return {
      ...row,
      manageStock: 1,
      stocks: stockOptions.length > 0 ? stockOptions : [selected],
      selectedStock: [selected],
      qty: selected.qty ?? row.qty,
      price: row.price ?? selected.mrp,
      dp: selected.dp ?? row.dp,
      unitCost: selected.unitCost ?? row.unitCost,
      productLineID: selected._id,
      total: (row.qtySold || 1) * (row.price ?? selected.mrp),
    };
  };
  ...
  const hydrateProductsWithStock = async (rows) => {
    return Promise.all(
      rows.map(async (row, rowIndex) => {
        ...
        if (!stocks.length && row.productLineID) {
          const selected = buildStockOptionFromSaleLine(row);
          return {
            ...row,
            manageStock: 1,
            stocks: [selected],
        ...
        return applyStockToRow(row, stocks, rowIndex, rows);
      }),
    );
  };
  // Fetch products
  ...
  const fetchSaleForEdit = async () => {
    ...
    setGlobalLoader(true);
    try {
      const res = await api.get(`/SalesForEdit/${id}`);
      if (res.data.status !== "Success")
        return ErrorToast("Failed to load sale data");

      const data = res.data.data;

      // Basic fields
      if (data.note) setNote(data.note);
      if (data.CreatedDate) setPurchaseDate(new Date(data.CreatedDate));
      if (data.discount) {
      ...
      console.error(err);
      ErrorToast("Something went wrong loading sale data");
    } finally {
      setGlobalLoader(false);
    }
  };
  ...
  useEffect(() => {
    ...
    (() => {
      Promise.all([fetchAllAccounts(), fetchSR()]).then(() => {
        fetchSaleForEdit();
      });
    })();
  ...
  const refreshStock = async (idx) => {
    ...
    try {
      const stocks = await fetchProductStocksByProductID(product, {
        silent: true,
      });

      if (stocks.length === 0 && !product.productLineID) {
        toast.error("No stock found for this product");
        return;
      }
      ...
      setSelectedProducts((prev) =>
        prev.map((p, i) => {
          if (i !== idx) return p;
          if (stocks.length === 0 && p.productLineID) {
            const fallback = buildStockOptionFromSaleLine(p);
            return {
              ...p,
              manageStock: 1,
              stocks: [fallback],
              selectedStock: [fallback],
            };
          }
          return applyStockToRow(p, stocks, idx, prev);
        }),
      );
    } catch (error) {
      console.error(error);
  ...
  const handleSubmit = async () => {
    ...
    const missingSerialProduct = selectedProducts.find(
      (p) => productHasSerialUI(p) && getValidSelectedSerials(p).length === 0,
    );
    ...
    const payload = {
      ...
      SaleProduct: selectedProducts.map((p) => ({
        ...
        name: p.name,
        ...(getValidSelectedSerials(p).length > 0
          ? { serialNos: getValidSelectedSerials(p).map((s) => s._id) }
          : {}),
        warranty: p.warranty,
      })),
    };
    ...
    try {
      setGlobalLoader(true);
      const res = await api.post(`/EditSale/${id}`, payload);

      if (res.data.status === "Success") {
        navigate(`/SaleList`);
      } else {
        ErrorToast(res.data.message || "Failed to update sale");
      }
    } catch (error) {
      ErrorToast("Something went wrong");
      console.error(error);
                      ...
                      {productHasSerialUI(p) ? (
                        <div className="flex justify-center">
                          <input
                            ...
                            className="global_input w-24 text-center cursor-not-allowed"
                          />
                        </div>
                      ) : p.manageStock === 0 ? (
                        <div className="flex items-center">
                          <button
                            ...
                            onClick={() => handleProductQtyDecrease(idx)}
