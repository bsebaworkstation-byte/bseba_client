...
const ReceiveInvestmentModal = ({ open, onClose, investor, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await api.post("/RecevedInvestment", {
      ...
      if (res.data.status === "success") {
        SuccessToast(res.data.message || "Amount received successfully");
        onSuccess?.();
        onClose();
      } else {
        ErrorToast(res.data.message || "Failed to receive investment");
      }
    } catch (err) {
      ErrorToast(
        err.response?.data?.message || "Failed to receive investment",
      );
    } finally {
  ...
  return (
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="receive-investment-title"
      ...
      >
        <h2
          id="receive-investment-title"
          className="text-lg font-semibold mb-1"
        >
          {heading("receiveInvestment")}
        </h2>
...
export default ReceiveInvestmentModal;
