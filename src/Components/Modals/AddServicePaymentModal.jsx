...
const AddServicePaymentModal = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await api.post(`/AddServicePayment`, {
        serviceID: service._id,
        payment,
      });

      if (res.data.status === "Success") {
        SuccessToast(res.data.message || "Payment added successfully");
        onSuccess?.();
        onClose();
      } else {
        ErrorToast(res.data.message || "Failed to add payment");
      }
    } catch (err) {
      console.error(err);
      ErrorToast(err.response?.data?.message || "Failed to add payment");
    } finally {
